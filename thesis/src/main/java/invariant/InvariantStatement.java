...
public class InvariantStatement {
	 ...
	 * @param subSpecReq
	 */
	public InvariantStatement(String actName, Integer posAction, List<String> accSeqAction, String subSpecName,
	...
	/**
	 * Generate "unused" action invariant. That is, generates an invariant that blocks an action "actionName" from
	 * occurring at state "posAction". We need the name of the sub-specification in order to specify the position, as
	 * our states are called "s1" or "s2" or so on for <i>all</i> our sub-specifications. <b> Note: </b>Values of states
	 * are incremented by one in order to account for CIF3's insistence of starting from "s1" when minimizing automata.
	 * 
	 * @param actionName
	 * @param posAction
	 * @param subSpecName
	 * @return
	 */
	public static String InvariantStatementUnusedBuild(String actionName, Integer posAction, String subSpecName) {
	...
	/**
	 * Construct the invariant statement associated with the values stored in the object. <b> Note: </b>Values of states
	 * are incremented by one in order to account for CIF3's insistence of starting from "s1" when minimizing automata.
	 */
	public String buildInvariantStatement() {
		...
		while (processIterator.hasNext()) {
			...
			inv = inv + "and ( ";
			Iterator<Integer> stateIterator = subSpecRequirements.get(process).iterator();
