...
//			CIFWriter.writeCIF(dfa, new FileOutputStream(options.getOutFile()), "monolithic_controller");
//			PrintWriter write = new PrintWriter(options.getOutFile().replace("cif", "dot"));
...
//			CIFWriter.writeCIF(dfa, new FileOutputStream(cif_file), "monolithic_controller");
//		}
//			break;
//		case ("complement"): {
//			CompactDFA<String> dfa = BharatCustomCIFReader.readCIF(options.getInFiles().get(0));
//			for (String x : BharatCustomCIFReader.readCIF(options.getInFiles().get(1)).getInputAlphabet()) {
//				dfa.addAlphabetSymbol(x);
//			}
//			dfa = DFAs.complement(dfa, BharatCustomCIFReader.readCIF(options.getInFiles().get(1)).getInputAlphabet());
//			String[] outName = options.getInFiles().get(0).replace(".cif", "").split("/");
//			CIFWriter.writeCIF(dfa, new FileOutputStream(options.getOutFile()), outName[outName.length - 1]);
//		}
//			break;
//		case ("complete"): {
//			CompactDFA<String> dfa = BharatCustomCIFReader.readCIF(options.getInFiles().get(0));
...
//			AldebaranUtil.writeAldebaran(dfa, new FileOutputStream(options.getOutFile() + ".aut"));
//			String[] outName = options.getInFiles().get(0).replace(".cif", "").split("/");
//			CIFWriter.writeCIF(dfa, new FileOutputStream(options.getOutFile()), outName[outName.length - 1]);
//		}
//			break;
//
//		case ("consistent"): {
//			CompactDFA<String> dfaSpecification = BharatCustomCIFReader.readCIF(options.getInFiles().get(0));
//			Map<String, CompactDFA<String>> subSpecificationsMap = Misc
//					.generateSubSpecificationsMap(options.getInFiles());
//			dfaSpecification = Consistency.check(options.getInFiles().subList(1, options.getInFiles().size()),
//					dfaSpecification, subSpecificationsMap);
//			CIFWriter.writeCIF(dfaSpecification, new FileOutputStream(options.getOutFile()), "specification");
...
//			break;
		...
		case ("xguards"): {
			...
			if (!equal) {
				...
				// StateEquivalence.calculateEquivalentStates(dfaSpecification, subSpecificationsMap)));
//				System.in.read();
				Set<String> prefActions = new HashSet<>();
				// prefActions.add("switchA");
				// prefActions.add("switchB");
				// GlobalMonitor gm = new GlobalMonitor(options, dfaSpecification, cons, subSpecificationsMap,
		...
		case ("mguards"): {
			...
			// Paths.get(options.getInFiles().get(0).replace(".", "_bkp.")), StandardCopyOption.REPLACE_EXISTING);
			// CIFWriter.writeCIF(dfaSpecification, new FileOutputStream(options.getInFiles().get(0)), "specification");
			// if (!consistent) {
			// return;
			// }
			// // Calculate the equivalent state map
			...
			// System.out.println(x + " : " + subSpecificationsMap.get(x).getInputAlphabet());
			// }
			Map<String, Map<Integer, Map<String, Set<Integer>>>> constraints = StateGuards
					.execStateGuards(dfaSpecification, subSpecificationsMap);
			// Put the constraints in the "Constraints" class
			...
			if (!equal) {
				System.out.println("Just state guards are not enough, beginning with monitor computation");
				Differentiator diff = new Differentiator(dfaSpecification, nonInjectionMapping(
						StateEquivalence.calculateEquivalentStates(dfaSpecification, subSpecificationsMap)));
				System.out.println("Calculated equivalent states");
				System.err.println(nonInjectionMapping(
						StateEquivalence.calculateEquivalentStates(dfaSpecification, subSpecificationsMap)));
				Map<List<Integer>, CompactDFA<String>> monitorMaps = diff.generateMonitors();
				System.out.println(monitorMaps);
				for (List<Integer> p : monitorMaps.keySet()) {
					System.out.print(p);
					CompactDFA<String> i = monitorMaps.get(p);
					System.out.println(" " + i.size());
					// for (Integer state : i.getStates()) {
					// for (String in : i.getInputAlphabet()) {
					// Integer dState = i.getSuccessor(state, in);
					// if (null != dState) {
		...
		// for (int i = 1; i < options.getInFiles().size(); i++) {
		// String name = options.getInFiles().get(i);
		// name = name.split("\\\\")[name.split("\\\\").length - 1].replace(".cif", "");
		// System.out.println(name);
		// subSpecificationsMap.put(name, BharatCustomCIFReader.readCIF(options.getInFiles().get(i)));
		// }
		// Invariant inv = new Invariant(BharatCustomCIFReader.readCIF(options.getInFiles().get(0)),
		// subSpecificationsMap);
		// inv.computeInvariants();
		// // inv.getConstraints().forEach(x -> System.out.print(x));
		// FileOutputStream stream = new FileOutputStream(options.getOutFile(), false);
		// writeDecomposition(options, options.getInFiles().subList(1, options.getInFiles().size()), stream,
		// inv.getConstraints(), false, null);
		// // Copy the original file
		// Files.copy(Paths.get(options.getOutFile()), Paths.get(options.getOutFile().replace(".", "gen.")),
		...
		// StandardCopyOption.REPLACE_EXISTING);
		// inv.generateInitialMonitor();
		// CompactDFA<String> monitor = inv.getMonitor();
		// System.out.println("size of the monitor: " + monitor.size());
		// writeDecomposition(options, options.getInFiles().subList(1, options.getInFiles().size()), stream,
		// inv.getConstraints(), true, monitor);
		// boolean equal = true;
		// do {
		// if (!inv.generateNewMonitor(equal)) {
		// break;
		// }
		// monitor = inv.monitor;
		// System.out.println("size of the monitor: " + monitor.size());
		...
		// StandardCopyOption.REPLACE_EXISTING);
		// CIF3operations.exploreStatespaceCIF(options.getOutFile());
		// equal = CIF3operations.checkEquivalenceCIF(filesToCheck);
		// // writeDecomposition(options, options.getInFiles().subList(1, options.getInFiles().size()),
		// // stream, inv.getConstraints(), true, monitor);
		// } while (true);
		...
		case ("sanity"): {
			List<String> listTraces = BharatXESReader.readXES(options.getInFiles().get(1));
			listTraces.removeIf(x -> x.trim().isEmpty());
			CompactDFA<String> dfa = BharatCustomCIFReader.readCIF(options.getInFiles().get(0));
			FileOutputStream outStream = new FileOutputStream(options.getOutFile());
			List<Word<String>> wordList = new LinkedList<>();
			wordList = BharatXESReader.getTraces(listTraces, dfa.getInputAlphabet());
			try (Writer writer = new BufferedWriter(new OutputStreamWriter(outStream))) {
				for (Word<String> input : wordList) {
					writer.write(dfa.accepts(input) + " : " + input + "\n");
				}
			}
		}
			break;
	...
	// * @throws IOException
	// */
	// private static void writeStateGuardsToFile(Args options, StateGuards guards, List<String> outNames,
	...
	// writer.write("\n");
	// }
	// List<String> fileContents = FileUtils.readLines(new File(options.getOutFile() + "_delete"), "utf-8");
	// fileContents.forEach(x -> {
	// try {
	// writer.write(x + "\n");
	// } catch (IOException e) {
