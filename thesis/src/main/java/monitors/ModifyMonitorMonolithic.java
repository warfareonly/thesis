...
public class ModifyMonitorMonolithic {
	...
	CompactNFA<String> monitor = new CompactNFA<String>(new SimpleAlphabet<String>());

	/**
	 * Constructor which basically does two things: copies the incoming monitor inside and then deletes the passed
	 * transition from the copy of the monitor. Transition here meaning the final action of the transition sequence,
	 * since the earlier symbols comprise the sequence used to "access" the final action. It is not meant to call any
	...
	/**
	 * Check if the transition specified by the parameters is the only transition between the source and destination
	 * states. If it is, then we will need to merge the source and destination states into a single state, as otherwise
	 * the destination state will be unreachable!
	 * 
	 * @param sourceState
	 * @param action
	 * @param destinationState
	 * @return boolean whether there is a single transition (true) or not (false).
	 */
	...
	/**
	 * Determinize and minimize the NFA monitor into a deterministic NFA. Thus, even though the monitor is stored as an
	 * NFA, it is still deterministic.
	 * 
	 * @param nfa
	 */
	private void determinizeAndMinimize(CompactNFA<String> nfa) {
		FastDFA<String> mid = new FastDFA<String>(this.monitor.getInputAlphabet());
		NFAs.determinize(nfa, this.monitor.getInputAlphabet(), mid, true, false);
		// Determinization and minimization complete, and the result is stored in "mid".
		this.monitor.clear();
		this.monitor = new CompactNFA<String>(this.monitor.getInputAlphabet());
		AutomatonLowLevelCopy.copy(AutomatonCopyMethod.BFS, mid, this.monitor.getInputAlphabet(), this.monitor);
	...
	private void mergeStates(Integer sourceState, Integer destinationState) {
		...
		incomingTransitions.addAll(StateInformation.getPredecessors(this.monitor, destinationState));
		outgoingTransitions.addAll(StateInformation.getSuccessors(this.monitor, destinationState));
		...
		for (Integer srcState : this.monitor.getStates()) {
			for (String input : this.monitor.getInputAlphabet()) {
				...
				if (null != this.monitor.getSuccessors(srcState, input)) {
					if (this.monitor.getSuccessors(srcState, input).size() > 0) {
						destState = (Integer) this.monitor.getSuccessors(srcState, input).toArray()[0];
	...
	/**
	 * Removes a transition from the <b>sourceState</b> via <b>input</b> going to <b>destinationState</b>
	 * 
	 * @param sourceState
	 * @param input
	 * @param destinationState
	...
	private void removeTransitionNFA(Integer sourceState, String input, Integer destinationState) {
		// Even though there is a for-loop here, we always have a deterministic NFA, so it does not
		// matter.
	...
	/**
	 * Adds a transition from the <b>sourceState</b> via <b>input</b> going to <b>destinationState</b>
	 * 
	 * @param sourceState
	 * @param input
	 * @param destinationState
	...
	public CompactDFA<String> getMonitor() {
		CompactDFA<String> ret = new CompactDFA<String>(this.monitor.getInputAlphabet());
		Mapping<Integer, Integer> x = AutomatonLowLevelCopy.copy(AutomatonCopyMethod.BFS, this.monitor,
