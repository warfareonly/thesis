...
public class Args {
	...
	@Parameter(names = { "--command", "-c" }, description = "command to execute", validateWith = CommandValidate.class)
	...
	@Parameter(names = { "--inFiles", "-i" }, description = "List of input files", variableArity = true)
	private List<String> inFiles = new ArrayList<String>();

	@Parameter(names = { "--outFile", "-o" }, description = "Output file", validateWith = OutFileValidate.class)
	private String outFile = "default.out";

	public void validateOptions() {
		if (command.equalsIgnoreCase("complement") && inFiles.size() != 2) {
			throw new ParameterException("Complement operation must have a model and a reference");
		}
